...
function revDel(options, cb) {
	...
	if (options.newManifest) {
		...
		var oldFiles = getChanged(oldManifest, newManifest, options);
...
function getChanged(oldObject, newObject, options) {
    var manifest = path.join(options.base, options.dest);
    var dirname = path.dirname(manifest);
    var basename = path.basename(manifest);
    var files = fs.readdirSync(dirname);

    var arrNewObj = [];
    Object.keys(newObject).map(function(k) { arrNewObj.push(k); arrNewObj.push(newObject[k]); return newObject[k] });
    var bundle = arrNewObj[0];
    var bundleV = arrNewObj[1];

    var result = [];
    files.map(function(obj){
        if(obj.search(bundle.split(".")[0])>=0 && obj != basename && obj != bundle && obj != bundleV){
            result.push(path.join(dirname, obj));
        }
    });
    return result;
}
